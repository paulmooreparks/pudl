...
2. Update the version everywhere it appears: the header comment in `pudl.css`, the version link in the topbar of `reference.html`, and the "Status" section of `README.md`.
...
7. Record the integrity hashes. Fetch each file from jsDelivr, confirm it matches the tagged copy, and compute its hash:
   ...
   ```sh
   for f in pudl.css pudl-theme.js pudl-windows.css pudl-windows.js; do
